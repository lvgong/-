...
static const double bloating = 0.1; //膨胀范围-------------
/// [m] --- The width of the vehicle
static const double width = 0.58 + 2 * bloating;//车的宽度-------------
/// [m] --- The length of the vehicle
static const double length = 0.72 + 2 * bloating;//车的长度-------------
...
static const float cellSize = 0.1; //在2D网格中cell的大小，地图分辨率-------------
/*!
...
//用于预计算所有可能的转向的框的大小-------------一定要能容纳小车旋转下的各种情况
// static const int bbSize = std::ceil((sqrt(width * width + length* length) + 0.4) / cellSize);
static const int bbSize = std::ceil((sqrt(width * width + length* length)) / cellSize);
/// [#] --- The sqrt of the number of discrete positions per cell
static const int positionResolution = 1;//每个cell里的离散位置数量的平方根-------------我们只用1格，因为我们的分辨率足够低
...
struct relPos {//相对于模板中心的位置，程序中以模板中心为坐标原点--------------
...
struct config {//用以获取每个theta角下覆盖栅格表的结构体------------------
  /// the number of cells occupied by this configuration of the vehicle
  int length;//该theta下模板中被设置为车轮廓或内部的数量
  /*!
     \var relPos pos[64]
     \brief The maximum number of occupied cells
     \todo needs to be dynamic----------就要这样
  */
  // relPos pos[64];//这里为什么是64有待考证----考证啥呀，就是不严谨
  std::vector<relPos> pos; //可以动态调整大小
};
