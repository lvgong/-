...
namespace planning
{
    ...
    void DynamicHybridAstar::setMapCallBack(const nav_msgs::OccupancyGrid::Ptr grid) {
        ...
        //--------------不使用DP_map
        // grid_search_ptr_ = make_unique<GridSearch>(grid_);//智能指针
        // dynaminc_grid_search_ptr_.reset(new GridSearch(grid));

        // reed_shepp_generator_ =  std::make_shared<ReedShepp>(0.3, 0.7);//RS   （最大曲率，步长）
        //这个2.2（曲率半径）要设置合理-----------先不用曲线直连
        // dubins_generator_ =  std::make_shared<DubinsStateSpace>(2.2, path_step_size);//db曲线
        // dynaminc_dubins_generator_ =  std::make_shared<DubinsStateSpace>(0.8, path_step_size);//db曲线
        // dynaminc_dubins_generator_ =  std::make_shared<DubinsStateSpace>(0.8, path_step_size);//db曲线
...
auto DynamicHybridAstar::HybridAstarSearch(Vec3d start_pose, Vec3d goal_pose) -> nav_msgs::Path {
    ...
    if(validMap){
        ...
        if (plan(start_pose, goal_pose, result)) {
            for (int i = 0; i < result.x.size(); ++i) {
                ...
                pathVehicle.pose.orientation.z = std::sin(o / 2);//朝向
    ...
    //但这样会在衔接处曲率突变，需要后续优化
            ...
            //如果当前节点离终点很近，就退出
            // 这里可以模糊一点，因为转换工作状态那里，小车位姿到了终点附近时就会切换工作状态，不会走到终点才切换
            // if(std::abs(cur_node->getX() - goal_node_->getX()) < 0.2 //当前节点的最后一个子位姿点的x坐标
...
/* 检查当前节点到终点是否能通过RS曲线连接生成无碰撞路径AnalyticExpansion，如果可以则找到完整路径，break
    否则在close_set_加入current_node*/
            ...
            for (int i = 0; i < next_node_num_dy; ++i) {//next_node_num_dy=6，6个朝向
    ...
    //取值范围其实是[-next_node_num_dy, next_node_num_dy]，在正向或反向下，
    //能取next_node_num_/2个有效值。
    //即，把[-next_node_num_dy, next_node_num_dy]分为（next_node_num_/2-1）份
    ...
    vector<Vec2d> DynamicHybridAstar::calculateCarBoundingBox(Vec3d pose) {
        ...
        const double shift_distance = length_ / 2 - back_edge_to_center_;//后轴中心对车中心的偏移
        //小车中心
