...
namespace chibot::slam {
    ...
    struct MappingParameters {
        ...
        Eigen::Vector2d map_origin_1, map_size_1;//地图尺寸

        Eigen::Vector3d map_min_boundary_, map_max_boundary_;  // map range in pos
        Eigen::Vector2d map_min_range_, map_max_range_;  // map range in pos

        Eigen::Vector3i map_voxel_num_;                        // map range in index
        // Eigen::Vector2i是Eigen库中用于表示两个整数的2维向量类型
        Eigen::Vector2i map_grid_num_;

        Eigen::Vector3i map_min_idx_, map_max_idx_;
        // Vector3d代表一个包含三个双精度浮点数的向量
        Eigen::Vector3d local_update_range_;
        double resolution_, resolution_inv_;
        double obstacles_inflation_;//障碍物栅格的膨胀距离，一般为小车尺寸
        string frame_id_;
        int pose_type_;
        ...
        int local_map_margin_;//表示局部栅格地图的更新和清除范围,默认值为10(10*0.1=1m)
    ...
    struct MappingData {
        ...
        // local_bound_min_和local_bound_max_都是三维向量，保存每个维度上更新栅格的范围
        Eigen::Vector3i local_bound_min_, local_bound_max_;
        // local_bound_min_和local_bound_max_都是三维向量，保存每个维度上更新栅格的范围
        Eigen::Vector2i local_range_min_, local_range_max_;
    ...
    * ============================== */
    ...
    inline int SDFMap::toAddress(const Eigen::Vector2i& id) {
        return id(0) * mp_.map_grid_num_(1) + id(1);
    }

    //二维向量 ---> 一维向量，转为索引值
    inline int SDFMap::toAddress(int& x, int& y) {
        //算法首先按列算
        return x * mp_.map_grid_num_(1) + y ;
    }

    inline void SDFMap::boundIndex(Eigen::Vector2i& id) {
    Eigen::Vector2i id1;
    id1(0) = max(min(id(0), mp_.map_grid_num_(0) - 1), 0);
    id1(1) = max(min(id(1), mp_.map_grid_num_(1) - 1), 0);
    id = id1;
    }
    ...
    //膨胀
    inline void SDFMap::inflatePoint(const Eigen::Vector2i& pt, int step, vector<Eigen::Vector2i>& pts) {
        int num = 0;
