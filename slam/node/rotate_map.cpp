...
namespace chibot::slam {
	...
	void RotateMap::mapTimerCallback(const ros::TimerEvent& event) {
		...
		nav_msgs::OccupancyGrid map = carto_map_;//获取原始地图
		map2Img(carto_mat);//地图数据转图像
		getTheta(carto_mat);
		rotMap(map);//旋转地图
	...
	void RotateMap::map2Img(cv::Mat& mat) {
		...
		for (uint32_t r = 0; r < sizey; ++r) {
			for (uint32_t c = 0; c < sizex; ++c) {
				// 根据地图数据的值判断像素的颜色。如果地图数据的值小于 75，则将对应的像素值设为 0（黑色），否则将像素值设为 carto_map_.data 中对应位置的值
	...
	//使用Canny边缘检测和霍夫变换方法找到地图上最长的一条直线，并计算其长度和角度
	void RotateMap::getTheta(const cv::Mat& mat) {
		static double maxLength = 30.0; // 0.1 m/pixel
		cv::Mat mid;//中间图像
		cv::Canny(mat, mid, 75, 99, 3);//对输入的mat图像进行边缘检测。这里使用阈值 75 和 99 进行边缘检测
		std::vector <cv::Vec4i> lines;//存储霍夫变换检测到的直线
...
int main(int argc, char** argv) {
	...
	chibot::slam::RotateMap _rotate_map;
	ros::spin();
	return 0;
}