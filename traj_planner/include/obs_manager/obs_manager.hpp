...
struct obstacle_traj      
{
  ...
  Eigen::Vector2d pos_xy_;  // 轨迹参数，位置
  double slower_;           // 轨迹参数，速度
  double offset_;           // 轨迹参数，角度偏转
...
public:
  ...
  void init(ros::NodeHandle &nh)   // Obs_Manager初始化
  {
    ...
    nh.param("/obs_Manager_node/pre_step_", pre_step, 30);//N，同MPC-CBF
    nh.param("/obs_Manager_node/step_time_", step_time, 0.15);//dt，同MPC-CBF

    // 使用障碍物轨迹真值或障碍物预测轨迹
    if (is_use_GroundTruth) {//使用障碍物轨迹真值
 ...
 void run(ros::NodeHandle &nh)   // Obs_Manager
 {
  ...
  // 根据时间返回障碍物的状态: posVel_list: 位置：P_x, P_y, 速度:V_x, V_y;   radius_lists: 对应的障碍物半径
  void get_obs_state(ros::Time cur_time, std::vector<Eigen::Vector4d>& posVel_list, std::vector<double>& radius_lists)
  {
    if (is_use_GroundTruth)
    {
      ...
      for (auto traj_iter = obstalce_trajs_.begin(); traj_iter != obstalce_trajs_.end(); traj_iter++) {
        // 计算当前时间与轨迹起始时间的时间差time_out
        double time_out = (cur_time - traj_iter->second.start_time).toSec();
        // 如果time_out大于等于5.0秒减去一个小数（1e-3），则说明该障碍物轨迹已经超时，将跳过不进行计算
        if(time_out >= 5.0 - 1e-3) {  // 超时的无用障碍物轨迹，删除并跳过
          continue;
        }
        // 对于未超时的障碍物轨迹，计算当前时间与轨迹起始时间的相对时间rel_time
        ...
        Eigen::Vector4d cur_posVel;//存储位置速度
...
private:
  ...
  // 移动轨迹可视化
  void show_Obs_traj() {
    // 检查obstalce_trajs_哈希表中是否包含障碍物轨迹，如果没有则直接返回，不进行可视化操作
    ...
    // 设置obs_ball的一些属性：frame_id为"world"，类型为SPHERE_LIST（球体列表），添加动作为ADD，ID为0，显示时间为0.20秒，颜色为蓝绿色
    obs_ball.header.frame_id = "map";
    ...
    obs_ball.id = 0;
    obs_ball.lifetime = ros::Duration(0.20); //0.2s后消失
    ...
    // 在一个时间范围内（从当前时刻开始，每次增加0.1秒，最多到2秒），获取当前时间点障碍物的状态和半径信息，并将其存储在posVel_list和radius_list中
    for (double add_time = 0.0; add_time < 7.8; add_time += 0.3) {
      ros::Time time_index = time_now + ros::Duration(add_time);

      std::vector<Eigen::Vector4d> posVel_list;   // 获取当前时间的障碍物状态
      std::vector<double> radius_list;            // 获取当前时间的障碍物半径

      // 遍历每个障碍物的位置和半径信息，将其转换为geometry_msgs::Point类型的点并添加到obs_ball.points中，同时设置obs_ball的尺寸大小为障碍物半径的两倍
      get_obs_state(time_index, posVel_list, radius_list);
      for (int i = 0; i < posVel_list.size(); i++) {
        geometry_msgs::Point p;
        p.x = posVel_list[i].x();//第1个元素就是x()
        p.y = posVel_list[i].y();//第2个元素
        p.z = 0.25;
        obs_ball.points.push_back(p);
        obs_ball.scale.x = radius_list[i]*2;//半径*2
        obs_ball.scale.y = radius_list[i]*2;
        obs_ball.scale.z = radius_list[i]*2;
      }
      // 将填充好数据的obs_ball添加到obs_balls_msg.markers中，以便在RViz中显示所有时间点的障碍物球体
  ...
  void pub_DCBF_traj() {
    ...
    acbf_msgs.data.resize(7 * N_ * get_obs_size()); //7个参数，N个步长

    for (int i = 0; i < N_; i++) {                // 预测第i步
      double add_time = (double)(i * delta_t_);
      ros::Time time_index = time_now + ros::Duration(add_time);//计算当前步的时间戳
      std::vector<Eigen::Vector4d> posVel_list;   // 获取当前时间的障碍物状态
      std::vector<double> radius_list;            // 获取当前时间的障碍物半径

      get_obs_state(time_index, posVel_list, radius_list);//获取当前时间的障碍物状态posVel_list和障碍物半径radius_list
      for (int j = 0; j < posVel_list.size(); j++) {  // 对于第j个障碍物
        ...
        acbf_msgs.data[ 7 * N_ * j + 7 * i + 2 ] = radius_list[j];      // 椭圆半长轴a，在这里长短轴一样
        acbf_msgs.data[ 7 * N_ * j + 7 * i + 3 ] = radius_list[j];      // 椭圆半短轴b
        acbf_msgs.data[ 7 * N_ * j + 7 * i + 4 ] = 0.0;                 // 椭圆方位角theta
        acbf_msgs.data[ 7 * N_ * j + 7 * i + 5 ] = posVel_list[j].z();  //速度x向             
        acbf_msgs.data[ 7 * N_ * j + 7 * i + 6 ] = posVel_list[j].w();  //速度y向       
      }
    }
    // 可以添加一个符号标志
  ...
  void obsTrajCallback(const dynamic_simulator::DynTraj& msg)     // 接收障碍物真值轨迹的回调函数
  {
    ...
    tmp_obs.Id_ = msg.id;
    tmp_obs.start_time = msg.header.stamp;//时间辍
    tmp_obs.Time0 = msg.start_time.stamp.toSec();//秒

    tmp_obs.scale_   << msg.s_num[0], msg.s_num[1]; //scale
    tmp_obs.pos_xy_  << msg.s_num[3], msg.s_num[4];//初始位置
    tmp_obs.slower_ = msg.s_num[6];//速度
    tmp_obs.offset_ = msg.s_num[7];//角度偏转
    tmp_obs.bbox_ << msg.bbox[0], msg.bbox[1];//尺寸

    Eigen::Vector2d bbox_half = tmp_obs.bbox_ / 2.0;  // 取bbox外接圆半径
    tmp_obs.circle_R_ = bbox_half.norm();

    // 更新障碍物轨迹容器
    // 检查obstalce_trajs_哈希表中是否已存在相同ID的障碍物轨迹，如果存在则替换该值，否则将新的障碍物轨迹插入到哈希表中
    ...
    // 遍历obstalce_trajs_哈希表中的每个障碍物轨迹，如果某个障碍物轨迹的时间超过了3秒，则将其加入待删除列表elements_to_remove
    for (auto traj_iter = obstalce_trajs_.begin(); traj_iter != obstalce_trajs_.end(); traj_iter++) {
    ...
    // 删除临时容器中的元素
    // 遍历待删除列表elements_to_remove，从obstalce_trajs_哈希表中删除相应的障碍物轨迹
