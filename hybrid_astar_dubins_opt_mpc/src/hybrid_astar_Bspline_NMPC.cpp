...
namespace chibot::topologyplanning {
  ...
  uint8_t* HybridAstarBsplineNMPC::cost_translation_ = nullptr;
  //增大ts，在同等路径下，要求完成路径所需速度更小，表现为“MPC/v_max”可以更小，tmp=traj_duration_越大
  //适当提高T：t_step，配合N可以延长预测路线长度
  double ts=0.8,dt = 0.1;//同定时周期
  #define N 25 //25步
  
  #define PI 3.1415926

  HybridAstarBsplineNMPC::HybridAstarBsplineNMPC()
     :line_ready_(false),
      // has_start(false),
      has_goal(false),
  ...
  //初始化
  void HybridAstarBsplineNMPC::init() {
      ...
      goal_sub_ = simple_nh.subscribe<geometry_msgs::PoseStamped>("goal", 1, boost::bind(&HybridAstarBsplineNMPC::goalCB, this, _1));
      
      ros::NodeHandle nh;//啥也不表示就是以节点命名的相对路径
      //订阅栅格地图，这个带有回调函数的必须放在主程序里，且快入快出
      map_sub_ = nh.subscribe("/map", 1, &HybridAstarBsplineNMPC::mapReceived, this);
      //设置拓扑起点集
      // start_sub = nh.subscribe("/initialpose", 1, &HybridAstarBsplineNMPC::startCallback, this);
      //设置拓扑终点集
      // goal_sub = nh.subscribe("/move_base_simple/goal", 1, &HybridAstarBsplineNMPC::goalCallback, this);

      //各种服务调用，服务器建立，很重要的一块
      task_srv_ = nh.advertiseService(TASK_SRV, &HybridAstarBsplineNMPC::task_service, this);

      vel_cmd_pub = nh.advertise<geometry_msgs::Twist>("/cmd_vel", 10);//发布速度指令
      
      //订阅代价地图，初始化或更新成本地图的信息
      costmap_sub_ = nh.subscribe(COSTMAP, 5, &HybridAstarBsplineNMPC::costmap_cb, this);
      //处理地图更新数据的回调函数，用于更新已存在的成本地图信息
      costmap_update_sub_ = nh.subscribe(COSTMAP_UPDATE, 5, &HybridAstarBsplineNMPC::costmap_update_cb, this);
      ...
      mpc_ptr.reset(new Mpc(N, dt));//Mpc构造函数

      //等获得了B样条轨迹后再开启定时器
      //MPC周期控制
      // cmd_timer = nh.createTimer(ros::Duration(0.05), &HybridAstarBsplineNMPC::cmdCallback, this);
      control_cmd_pub = nh.createTimer(ros::Duration(0.1), &HybridAstarBsplineNMPC::publish_control_cmd, this);//控制小车
   ...
   * 
   */
    void HybridAstarBsplineNMPC::run() {
        ...
        switch (cur_state_) {
          ...
          case StateValue::Pause:
            ...
            if(has_goal){
              ...
              //初始化控制点矩阵(2D)
              MatrixXd control_points = MatrixXd::Zero(poses.size(), 2);
 ...
 * @param msg 
 */
// void HybridAstarBsplineNMPC::startCallback(const geometry_msgs::PoseWithCovarianceStamped msg) {
 ...
 * @param msg 
 */
// void HybridAstarBsplineNMPC::goalCallback(const geometry_msgs::PoseStamped msg) {
//     if (cur_state_ != StateValue::Record_Traffic_route) return;
//     Vec3d goal_pose;
...
//描述终点位姿
// void HybridAstarBsplineNMPC::goalCallback(geometry_msgs::PoseStamped::ConstPtr const& msg) {
//     if (cur_state_ != StateValue::Run) return;
//     Vec3d goal_pose;
//     goal_pose[0] = msg->pose.position.x;
//     goal_pose[1] = msg->pose.position.y;
...
// }
  ...
  //转化为mbf的action_goal
  void HybridAstarBsplineNMPC::goalCB(const geometry_msgs::PoseStamped::ConstPtr& msg){
...
//接收地图信息
void HybridAstarBsplineNMPC::mapReceived(const nav_msgs::OccupancyGridConstPtr& msg) {
    //互斥锁
    ...
    //半结构化道路
    half_struct_planner_ = std::make_shared<HalfStructPlanner>(grid_);//创建HalfStructPlanner类
    half_struct_planner_->init();//初始化
}


 //各任务切换用服务器，服务器服务端
  auto HybridAstarBsplineNMPC::task_service(hybrid_astar_dubins_opt_mpc::chibot_task::Request& req, hybrid_astar_dubins_opt_mpc::chibot_task::Response& resp) -> bool {
...
//将存储的路点保存到文件里
auto HybridAstarBsplineNMPC::write_traffic_route(std::string& file_path, std::vector<Vec3d> const& line_start, std::vector<Vec3d> const& line_goal) -> bool {
...
auto HybridAstarBsplineNMPC::check_file_exist(std::string& file_path) -> bool {
...
//读取文件中的路点信息
auto HybridAstarBsplineNMPC::read_traffic_route(std::string& file_path, lines_type& lines) -> bool {
...
//读取路线
auto HybridAstarBsplineNMPC::read_file(std::string& file_path, vector<vector<Eigen::Vector3d>>& routes) -> bool {
  std::ifstream in(file_path.c_str());
  if (!in.is_open()) {
    ROS_ERROR("Open file %s failed!", file_path.c_str());
    return false;
...
因此，nav_msgs::OccupancyGrid::data中的值范围应该是-1到100之间。*/
  void HybridAstarBsplineNMPC::costmap_cb(nav_msgs::OccupancyGrid::ConstPtr const& msg) {
...
// 双重循环遍历收到的地图数据，根据其中的值来设置成本地图中对应位置的代价值。如果地图数据中的值小于0，表示该位置未知，将其设置为costmap_2d::NO_INFORMATION；否则根据cost_translation_映射将其设置为对应的代价值
  ...
  //处理地图更新数据的回调函数，用于更新已存在的成本地图信息
  void HybridAstarBsplineNMPC::costmap_update_cb(map_msgs::OccupancyGridUpdate::ConstPtr const& msg) {
...
// 通过双重循环遍历接收到的地图更新数据，并根据数据内容更新成本地图中对应位置的代价值。根据地图更新数据中的值来设置成本地图中对应位置的代价值
  ...
  //求机器人在地图中的位姿
  auto HybridAstarBsplineNMPC::robot_pose() -> std::optional<geometry_msgs::Pose> {
 ...
 //执行示教路径，这里的index是示教路径中的，如果车中途暂停可以恢复到这个点（其他函数中修正index）
  auto HybridAstarBsplineNMPC::send_exe(size_t const& index) -> bool {
    //合法性校验
    ...
    exe_ctrl_->sendGoal(goal,
                        boost::bind(&HybridAstarBsplineNMPC::exe_done, this, _1, _2),
                        ExeCtrl::SimpleActiveCallback(),//默认的回调
                        ExeCtrl::SimpleFeedbackCallback());
    return true;
  }

  void HybridAstarBsplineNMPC::exe_done(const actionlib::SimpleClientGoalState& state,
  ...
  //运行态Run分支状态机
  void HybridAstarBsplineNMPC::running_state() {
  ...
  //复位所有，清除所有
  void HybridAstarBsplineNMPC::reset() {
  ...
  //终止
  void HybridAstarBsplineNMPC::stop() {
  ...
  //取消导航至目标点（点到点&执行示教路径）
  void HybridAstarBsplineNMPC::cancel_goal() {
    ...
    //速度归零
  void HybridAstarBsplineNMPC::pub_zero_vel() {
  ...
  auto HybridAstarBsplineNMPC::is_free(const geometry_msgs::PoseStamped& pose) const -> bool {
  ...
  auto HybridAstarBsplineNMPC::is_danger(const geometry_msgs::PoseStamped& pose) const -> bool {
  ...
  //获取某一个位姿点的代价值
  auto HybridAstarBsplineNMPC::pose_cost(const geometry_msgs::PoseStamped& pose) const -> uint8_t {
    if (!costmap_) {//还没有代价地图
  ...
  //path:路径点  index：路径索引
  auto HybridAstarBsplineNMPC::nearest_info(nav_msgs::Path const& path, size_t const& index, int lb, int rb, bool following)
  ...
  //计算两点之间的距离和角度差
  inline auto HybridAstarBsplineNMPC::distance(geometry_msgs::Pose const& a,
  ...
  //点到点导航，普通的规划+控制
  auto HybridAstarBsplineNMPC::send_goto(size_t const& index) -> bool {
    //index合法性校验
    if (index == std::numeric_limits<size_t>::max() || index > cur_route_.poses.size()) {
    ...
    goto_ctrl_->sendGoal(goal,
                        boost::bind(&HybridAstarBsplineNMPC::goto_done, this, _1, _2),//结束导航时调用
  ...
  void HybridAstarBsplineNMPC::goto_done(const actionlib::SimpleClientGoalState& state,
...
//MPC周期执行
void HybridAstarBsplineNMPC::publish_control_cmd(const ros::TimerEvent &e)
{
...
double t_k,t_k_1;
    ...
    if(conut1%15==0)//调试
    {
        cout<<"current vel: "<<control_cmd[0] <<"m/s"<<endl;
        cout<<"current w: "<<control_cmd[1]<<"rad/s"<<endl;
        conut1=0;
    }
